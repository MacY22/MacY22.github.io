...
static void entry_del(Entry *ent)
{
    ...
    if (too_big)
    {
        threadpool_add(g_data.tp, &entry_del_async, ent, 1);
    }
    else
    {
        entry_destroy(ent);
...
static bool try_fill_buffer(Conn *conn)
{
    ...
    // can not read data from connection
    if (rv == 0)
    {
        if (conn->rbuf_size > 0)
        {
...
int main()
{
    ...
    g_data.tp = threadpool_create(4, 1024, 1);
    ...
    std::vector<struct epoll_event> events(20);

    while (true)
    {
        events.clear();
        int timeout_ms = (int)next_timer_ms();
        int num = epoll_wait(epoll_fd, events.data(), 20, timeout_ms);
        if (num == -1)
        {
            die("epoll_wait()");
        }
        for (int i = 0; i < num; i++)
        {
            if (events[i].data.fd == fd)
            {
                ...
                epoll_ctl(epoll_fd, EPOLL_CTL_ADD, connfd, &new_event);
            }
            else
            {
